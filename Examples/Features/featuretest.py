...
if __name__ == '__main__':
    ...
    index_labels = [i for i in  dData['close']['XOM'].index]
    ...
    if bPlot:
        ...
        for i, fcFunc in enumerate(lfcFeatures[:-1]):
            ...
            print dData['close']['XOM'].index
            plt.plot( dData['close']['XOM'].index, dData['close']['XOM'].values, 'r-' )
            plt.xticks(rotation=45)
            plt.gca().xaxis.set_major_locator(plt.MultipleLocator(5))
            plt.gca().xaxis.set_minor_locator(plt.MultipleLocator(2))
            plt.subplots(2, sharex=False, sharey=False)
            plt.plot( dData['close']['XOM'].index, ldfFeatures[i]['XOM'].values, 'g-' )
            plt.gca().xaxis.set_major_locator(plt.MaxNLocator(prune='lower'))
            plt.savefig("xom_"+str(i)+".pdf", format="pdf")
